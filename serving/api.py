...
app = FastAPI(title="S&P 500 prediction API", version="1.0.0")
...
def load_active_model():
    ...
    try:
        ...
        # Models are registered under the name 'spy_price_predictor'
    ...
    try:
        ...
        if os.path.exists(run_id_file):
            ...
            # Find the path in the local mlruns folder
            # Experiment is usually '0' or we search for the run folder
            local_paths = [
...
def predict_price(request: PredictionRequest):
    """
    Generates next day's price prediction and registers it to the DB.
    """
    global model
    if model is None:
        # Try loading model again in case it was trained after startup
        load_active_model()
        if model is None:
            raise HTTPException(status_code=503, detail="Prediction model is not loaded/available.")
            
    # Prepare features vector (matching training features exactly)
    ...
    # Save prediction to DB predictions table
    conn = config.get_db_connection()
    try:
        cursor = conn.cursor()
        placeholder = "?" if config.DB_TYPE == "sqlite" else "%s"
        
        # Serialize input features to JSON string for SQLite, dict for PG
        input_features_serialized = json.dumps(features_df.iloc[0].to_dict())
        if config.DB_TYPE == "postgresql":
            # For Postgres JSONB, pass dict directly or cast
            input_features_serialized = features_df.iloc[0].to_dict()
        ...
        # We don't raise HTTP error here to ensure the user gets their prediction back
    finally:
        conn.close()
...
def get_metrics():
    ...
    directly from the actuals table in the database.
    """
    conn = config.get_db_connection()
    try:
        query = "SELECT date, actual_price, predicted_price, absolute_error, direction_correct FROM actuals ORDER BY date DESC LIMIT 30"
